...
// console.log('=======================================')
// console.log(`swAlign`, swAlign('Mushoku Tensei: Jobless Reincarnation Part 2', 'Mushoku Tensei: Isekai Ittara Honki Dasu Part 2', { alignment: 'local', equal: 2, align: -1, insert: -1, delete: -1 }))
// console.log(`nwAlign`, nwAlign('Mushoku Tensei: Jobless Reincarnation Part 2', 'Mushoku Tensei: Isekai Ittara Honki Dasu Part 2', { alignment: 'local', equal: 2, align: -1, insert: -1, delete: -1 }))
// console.log('=======================================')
...
export const getEpisode: GetEpisode['function'] = async (args) => {
  ...
  const mostCommonSubnames =
    findMostCommon(
      names
        .flatMap(name =>
          // names.flatMap(_name => getAlignedStringParts(name.name, _name.name))
          names.flatMap(_name => swAlign(name.name, _name.name, { alignment: 'local', equal: 2, align: -1, insert: -1, delete: -1 }))
        )
        .map(alignment => [alignment.alignedLeft, alignment.alignedRight, (alignment.originalLeft.length > alignment.alignedRight.length ? alignment.originalLeft.length : alignment.originalRight.length) / alignment.score])
        // .map(alignment => alignment.alignedSequences)
        .filter(([val, val2]) => val === val2)
        .map(([val]) => val)
        .filter(val => val.trim().length)
  ...
  console.log('test3',
    names
      ...
      .map(alignment => [alignment.alignedLeft, alignment.alignedRight, alignment.score / (alignment.originalLeft.length > alignment.alignedRight.length ? alignment.originalLeft.length : alignment.originalRight.length)] as const)
      .flatMap(([leftAlignment, rightAlignment, score]) =>
        // names.flatMap(_name => getAlignedStringParts(name.name, _name.name))
        // names.map(_name => [swAlign(leftAlignment, rightAlignment, { alignment: 'local', equal: 2, align: -1, insert: -1, delete: -1 }), score, leftAlignment, rightAlignment])
        names.map(_name => swAlign(leftAlignment, rightAlignment, { alignment: 'local', equal: 2, align: -1, insert: -1, delete: -1 }))
      )
  )
